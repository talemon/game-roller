...
### Named Rules
**The Traceability Rule.** A facet hue exists so that a chip can be traced back to the compartment that produced it. A new facet earns a hue because it needs identity in the results, never because the palette wants more variety. Hues are never reused across facets and never applied to anything that is not a result, a result's control, or a live slot's edge.
